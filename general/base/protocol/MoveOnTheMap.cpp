...
void MoveOnTheMap::newDirection(const Direction& the_new_direction)
{
    if (last_direction_is_set == false) {
        ...
        Logger::instance().log(Logger::Debug, std::stringLiteral("newDirection(%1)").arg(directionToString(the_new_direction));
    #endif

    if (last_direction != the_new_direction)
    {
        #ifdef DEBUG_MESSAGE_MOVEONTHEMAP
            Logger::instance().log(Logger::Debug, "send_player_move_internal((%1,%2)").arg(last_step).arg(directionToString(the_new_direction));
        #endif
...
void MoveOnTheMap::stopMove()
{
    ...
    switch(last_direction)
    {
        ...
        case Direction_move_at_bottom:
        case Direction_move_at_left:
            last_direction = (CatchChallenger::Direction)(last_direction - 4);
