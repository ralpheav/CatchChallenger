...
namespace CatchChallenger
{
    class DatapackGeneralLoader
    {
        public:
            ...
            #endif
            static std::pair<std::vector<const tinyxml2::XMLElement*>, std::vector<Profile>> loadProfileList(
            ...
            static std::vector<ServerSpecProfile> loadServerProfileListInternal(const std::string &datapackPath, const std::string &mainDatapackCode, const std::string& file);
                ...
                static std::unordered_map<uint16_t, Shop> preload_shop(const std::string& file, const std::unordered_map<uint16_t, Item>& items);
