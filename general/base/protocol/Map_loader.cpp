...
#include "util/cpp11addition.h"
#include "lib/tinyXML2/customtinyxml2.h"
...
bool Map_loader::tryLoadMap(const std::string& file, const bool& botIsNotWalkable)
{
                ...
                while (SubChild != NULL)
                {
                    if (SubChild->Attribute(XMLCACHEDSTRING_x) != NULL && SubChild->Attribute(XMLCACHEDSTRING_y) != NULL)
                    {
                        ...
                        else
                        {
                            ...
                            if (!ok) {
                                std::cerr << "Wrong conversion with y: child->Name(): " << SubChild->Name() << ", file: " << file << std::endl;
                            }
                    ...
                    int32_t decompressedSize = 0;
                        ...
                        if (strcmp(data->Attribute(XMLCACHEDSTRING_compression), XMLCACHEDSTRING_zlib) == 0) {
                            decompressedSize = decompressZlib(
                                    ...
                                    dataRaw.data(),
                                    static_cast<uint32_t>(dataRaw.size())
                            );
                        } else
                    #endif
                        {
    ...
    bool dirt = false;
    bool ledgesRight = false;
    bool ledgesLeft = false;
    bool ledgesBottom = false;
    bool ledgesTop = false;
    ...
    while (x < map_to_send_temp.width)
    {
        ...
        {
            if (WalkableBin != NULL) {
                walkable = WalkableBin[x * 4 + y * map_to_send_temp.width * 4 + 0] != 0x00
                        || WalkableBin[x * 4 + y * map_to_send_temp.width * 4 + 1] != 0x00
                        || WalkableBin[x * 4 + y * map_to_send_temp.width * 4 + 2] != 0x00
                        || WalkableBin[x * 4 + y * map_to_send_temp.width * 4 + 3] != 0x00;
            } else {
                walkable = false;
            }
            if (CollisionsBin != NULL) {
                collisions = CollisionsBin[x * 4 + y * map_to_send_temp.width * 4 + 0] != 0x00
                          || CollisionsBin[x * 4 + y * map_to_send_temp.width * 4 + 1] != 0x00
                          || CollisionsBin[x * 4 + y * map_to_send_temp.width * 4 + 2] != 0x00
                          || CollisionsBin[x * 4 + y * map_to_send_temp.width * 4 + 3] != 0x00;
            } else {
                collisions = false;
            }
            if (DirtBin != NULL) {
                dirt = DirtBin[x * 4 + y * map_to_send_temp.width * 4 + 0] != 0x00
                    || DirtBin[x * 4 + y * map_to_send_temp.width * 4 + 1] != 0x00
                    || DirtBin[x * 4 + y * map_to_send_temp.width * 4 + 2] != 0x00
                    || DirtBin[x * 4 + y * map_to_send_temp.width * 4 + 3] != 0x00;
            } else {
                dirt = false;
            }
            if (LedgesRightBin != NULL) {
                ledgesRight = LedgesRightBin[x * 4 + y * map_to_send_temp.width * 4 + 0] != 0x00
                           || LedgesRightBin[x * 4 + y * map_to_send_temp.width * 4 + 1] != 0x00
                           || LedgesRightBin[x * 4 + y * map_to_send_temp.width * 4 + 2] != 0x00
                           || LedgesRightBin[x * 4 + y * map_to_send_temp.width * 4 + 3] != 0x00;
            } else {
                ledgesRight = false;
            }
            if (LedgesLeftBin != NULL) {
                ledgesLeft = LedgesLeftBin[x * 4 + y * map_to_send_temp.width * 4 + 0] != 0x00
                          || LedgesLeftBin[x * 4 + y * map_to_send_temp.width * 4 + 1] != 0x00
                          || LedgesLeftBin[x * 4 + y * map_to_send_temp.width * 4 + 2] != 0x00
                          || LedgesLeftBin[x * 4 + y * map_to_send_temp.width * 4 + 3] != 0x00;
            } else {
                ledgesLeft = false;
            }
            if (LedgesBottomBin != NULL) {
                ledgesBottom = LedgesBottomBin[x * 4 + y * map_to_send_temp.width * 4 + 0] != 0x00
                            || LedgesBottomBin[x * 4 + y * map_to_send_temp.width * 4 + 1] != 0x00
                            || LedgesBottomBin[x * 4 + y * map_to_send_temp.width * 4 + 2] != 0x00
                            || LedgesBottomBin[x * 4 + y * map_to_send_temp.width * 4 + 3] != 0x00;
            } else {
                ledgesBottom = false;
            }
            if(LedgesTopBin != NULL) {
                ledgesTop = LedgesTopBin[x * 4 + y * map_to_send_temp.width * 4 + 0] != 0x00
                         || LedgesTopBin[x * 4 + y * map_to_send_temp.width * 4 + 1] != 0x00
                         || LedgesTopBin[x * 4 + y * map_to_send_temp.width * 4 + 2] != 0x00
                         || LedgesTopBin[x * 4 + y * map_to_send_temp.width * 4 + 3] != 0x00;
            } else {
                ledgesTop = false;
            }
            ...
            {
                if (MonsterCollisionBin.at(index)[x * 4 + y * map_to_send_temp.width * 4 + 0] != 0x00
                 || MonsterCollisionBin.at(index)[x * 4 + y * map_to_send_temp.width * 4 + 1] != 0x00
                 || MonsterCollisionBin.at(index)[x * 4 + y * map_to_send_temp.width * 4 + 2] != 0x00
                 || MonsterCollisionBin.at(index)[x * 4 + y * map_to_send_temp.width * 4 + 3] != 0x00)
                {
            ...
            if (Walkable.size() > 0) {
                map_to_send_temp.parsed_layer.walkable[x + y * map_to_send_temp.width]
                        = (walkable || monsterCollision) && !collisions && !dirt;
            }
            if (Dirt.size() > 0)
            {
                if (dirt)
    ...
    if (Walkable.size() > 0)
    {
        ...
        if (botIsNotWalkable)
        {
            ...
            while (index<map_to_send_temp.bots.size())
            {
                ...
                {
                    if (bot.property_text.find(CACHEDSTRING_skin) != bot.property_text.cend()
                            && (
                                bot.property_text.find(CACHEDSTRING_lookAt) == bot.property_text.cend()
                                || bot.property_text.at(CACHEDSTRING_lookAt) != CACHEDSTRING_move
                                )
                        ) {
...
std::vector<MapMonster> Map_loader::loadSpecificMonster(const std::string& fileName, const std::string& monsterType)
{
    ...
    if (layer != NULL)
    {
        ...
        while (monsters != NULL)
        {
            if (monsters->Attribute(XMLCACHEDSTRING_id) != NULL
                    && ((monsters->Attribute(XMLCACHEDSTRING_minLevel) != NULL
                    && monsters->Attribute(XMLCACHEDSTRING_maxLevel) != NULL)
                    || monsters->Attribute(XMLCACHEDSTRING_level) != NULL)
                    && monsters->Attribute(XMLCACHEDSTRING_luck) != NULL)
            {
